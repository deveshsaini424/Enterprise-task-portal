...
const KanbanColumn = ({ title, tasks, status, onOpenModal, onTaskClick }) => { // <-- Add onTaskClick
  ...
  return (
    ...
    <div className="flex-1 min-w-[300px] bg-gray-50 rounded-xl p-4 border border-gray-200">
      {/* Column Header */}
      <div className={`flex justify-between items-center mb-4 p-3 rounded-lg ${headerColor} border-t-4`}>
      ...
      <div className="space-y-4 h-[500px] overflow-y-auto pr-2">
        {tasks.length > 0 ? (
          ...
          <p className="text-sm text-gray-500 p-4 text-center">No tasks in this list.</p>
        )}
      </div>
...
const KanbanBoard = ({ tasks, project, onTaskCreated, onTaskUpdate, onTaskDelete }) => { // <-- ADD UPDATE/DELETE PROPS
